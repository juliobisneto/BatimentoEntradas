#!/usr/bin/env python3
"""
Script para gerar arquivos de teste de transações para janeiro de 2026
"""
import random
import csv
...
# Configurações
START_DATE = datetime(2026, 1, 10)  # 10 de janeiro de 2026
NUM_FILES = 10
RECORDS_PER_FILE = 50
...
def main():
    print("=" * 70)
    print("📁 GERADOR DE ARQUIVOS DE TESTE - JANEIRO 2026")
    print("=" * 70)
    print()
    ...
    total_value = sum(f['total_value'] for f in created_files)
    total_sales = sum(f['sales'] for f in created_files)
    total_refunds = sum(f['refunds'] for f in created_files)
    
    print(f"💰 Valor Total:       R$ {total_value:,.2f}")
    print(f"📈 Total de Vendas:   {total_sales}")
    print(f"📉 Total de Reembolsos: {total_refunds}")
    print()
